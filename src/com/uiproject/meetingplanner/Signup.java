...
public class Signup extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		signuppic = (ImageView) findViewById(R.id.signuppic);

		signuppic.setOnClickListener(new OnClickListener() {
	...
	public void submit(View button) throws JSONException, ParseException, UnsupportedEncodingException {
		...
		if (fname.length() == 0 || lname.length() == 0 || phone.length() == 0 || email.length() == 0 || pw.length() == 0 || pw2.length() == 0) {
	...
	private class StoreDataToDb extends AsyncTask<Integer, Void, Void> {
		...
		protected Void doInBackground(Integer... params) {
			...
			// db.createUser(uid, fname, lname, email,
			// Long.toString(phonenumber), 0, 0);
			db.close();
